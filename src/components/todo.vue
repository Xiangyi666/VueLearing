<template>
  <div class="page lists-show"><!--最外层容器-->
    ...
    <div class="content-scrollable list-items">
      <!--容器下半部分-->
      <div v-for="item in items" :key='item.text'> <!-- 这里`v-for`会循环我们在 `data`函数 事先定义好的 ’items‘模拟数据，循环后拿到单个对象，在通过prop把数据传输给子组件 item -->
        <!-- <item item=''> </item> -->
        <weiBox wei='item'></weiBox>
      </div>
      <cntButton msg='sonButton'></cntButton>
    </div>
...
import cntButton from './zujian/countButton'
import weiBox from './zujian/weiBox'
export default {
  data () {
    return {
      todo: { // 详情内容
      ...
      items: [ // 代办单项列表
        { checked: false, text: 'be regret to do all', isDelete: false },
        { checked: false, text: 'but again and again', isDelete: false },
        { checked: false, text: 'gain the place for artstation ', isDelete: false }
      ],
      text: '' // 新增代办单项绑定的值
    }
  },
